...
export async function sendWebSocketMessage(writer: BrowserMessageWriter, data: string | ArrayBuffer): Promise<void> {
    const content: TypstPreviewMessageContent = typeof data === "string" ? {
        format: "text",
        data: data
    } : {
        format: "binary",
...
export function isTypstPreviewPageInner(element: Element | null | undefined): element is TypstPreviewPageInner {
    return !!(
        element &&
...
export let ignoredEvent = (function () {
